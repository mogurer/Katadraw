...
func _draw_bg(vp: Vector2) -> void:
	"""背景画像を描画。テクスチャがない場合はBG_COLORでフォールバック"""
	if _game.bg_texture:
...
func _draw_logo(vp: Vector2) -> void:
	_game.draw_rect(Rect2(Vector2.ZERO, vp), Color(1.0, 0.937, 0.89))
...
func _draw_title(vp: Vector2) -> void:
	...
	var alpha: float = _crossfade_alpha() * fade
	...
	_game.draw_string(_game.font, Vector2(0, vp.y - 30), tr("TITLE_COPYRIGHT"), HORIZONTAL_ALIGNMENT_CENTER, vp.x, 32, Color(0.45, 0.38, 0.45, fade))
...
func _draw_menu(vp: Vector2) -> void:
	...
	var text_c := Color(0.45, 0.38, 0.45)
	var sel_c := Color(0.95, 0.19, 0.32)

	if _game.title_logo_texture:
		var tex_size: Vector2 = _game.title_logo_texture.get_size()
		var draw_w: float = vp.x * 0.85 * 1.2  # タイトル画面と同じ1.2倍
		var scale_f: float = draw_w / tex_size.x
		var draw_h: float = tex_size.y * scale_f
		var logo_cy: float = vp.y * 0.38 * 0.8  # タイトル画面と同じ20%上
	...
	var labels: Array[String] = [tr("MENU_GAME_START"), tr("MENU_CONFIG")]
	for i in range(2):
		var btn_center_y: float = (cy + 10.0 + i * 72.0) * 1.3 + i * 20.0  # 30%下へ + 間隔20
