...
| ステージセレクトで L/R ボタン | `select_prev_bgm()` / `select_next_bgm()` | インゲーム BGM **5 曲**を循環切り替え（イントロ先頭から再生。詳細は [6.5](#65-ステージセレクトでの-bgm-切り替え)） |
...
### 6.5 ステージセレクトでの BGM 切り替え

製品版のステージセレクト画面（`scenes/stage_select.gd`）で、プレイヤーがインゲーム BGM 曲を選べる。

| 項目 | 内容 |
|------|------|
| 操作 | ゲームパッド **L ボタン**（左ショルダー）→ 前の曲、**R ボタン**（右ショルダー）→ 次の曲 |
| 対象 | インゲーム BGM **5 曲**（`TRACKS[0]`〜`TRACKS[4]`：`01-05` / `01-06` / `01-08` / `02-03` / `02-09`） |
| 切り替え | 循環（末尾 ↔ 先頭）。`play_ingame()` を呼び、**イントロ（`_0000`）の先頭から** 再生 |
| 有効条件 | ステージ選択確認ポップアップ・タイトル戻り確認ポップアップが **非表示** のときのみ |
| 入力制限 | キーボード・マウスでの BGM 切り替え操作は **なし** |
| 選択の保持 | `_ingame_track_idx`（AutoLoad 内メモリ）。セーブデータには書き込まない。アプリ再起動で 0（`01-05`）に戻る |
...
| `select_prev_bgm()` | ステージセレクト: 前のインゲーム BGM へ（循環、先頭から再生） |
| `select_next_bgm()` | ステージセレクト: 次のインゲーム BGM へ（循環、先頭から再生） |
| `stop()` | 全停止 |
| `set_mute(muted)` | ミュート |
...
| `scenes/stage_select.gd` | タイトル戻り時の `stop()`、L/R による BGM 切り替え |
